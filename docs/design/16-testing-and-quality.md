...
- 스모크 결과는 감사로그에 `cassette_smoke` 이벤트로 남는다(스키마 소유: [03](03-data-and-persistence.md) §7.2 — **현재 03에 이 event_type이 없어 신설 요청 대상이다. §16 미해결 11**).
...
- `SimClock` 압축 재생이므로 실시간 대기는 0이다. 실제 `asyncio.sleep`을 쓰는 프로덕션 코드는 **주입된 대기 훅**을 경유해야 하고, 하네스는 그것을 즉시 반환하는 구현으로 교체한다. 다만 `Clock` ABC([02](02-domain-model.md) §8.1)에는 현재 대기 API가 없다 — 훅의 좌표(`Clock` 확장 vs `scheduler` 대기 유틸)는 **§16 미해결 항목 10**이며, 확정 전까지 하네스는 `asyncio.sleep`을 monkeypatch로 단축한다.
...
| 10 | **결정론 대기 훅의 좌표**(§7.4) | `Clock` ABC([02](02-domain-model.md) §8.1)에 대기 API가 없다. 02에 `sleep_until` 추가 또는 [12](12-scheduling-and-operations.md) 스케줄러의 대기 유틸 주입 중 택일 — **02·12와 조율 필요** | 확정 전까지 하네스가 `asyncio.sleep`을 monkeypatch |
| 11 | **`cassette_smoke` 감사 이벤트 스키마**(§5.6) | 감사로그 event_type 스키마의 정본은 [03](03-data-and-persistence.md) §7.2인데 현재 이 event_type이 없다 — **03에 신설 요청** | 미신설 시 스모크 결과가 감사로그가 아니라 리포트 파일로만 남는다 |
