...
Codex 공동 작성은 이름만 적지 않고 GitHub의 `codex` 계정에 연결되는 위 이메일을 반드시 포함한다. 트레일러는 커밋 메시지의 마지막 줄에 정확히 한 번 둔다.
...
|---|---|---:|---:|---:|---|
| **M0** 스캐폴드 (기반 계층) | S01~S08 | 54 | 16 | 30% | ◐ |
...
```
▶ 현재 단위 : S03-5 (계획 모델 RebalancePlan·TargetWeights·SanityResult)
  직전 완료 : S03-4 (Order·Fill 모델)
  다음 단위 : S03-6 (계좌·슬리브 core.accounts)
```
...
| **S02-2** | Decimal·화폐 규약 `core.money` | `src/omra/core/money.py`, `tests/unit/core/test_money.py` | `Dec`가 float·**bool** 입력 거부 · `to_text` 정규형 4규약(지수 표기 부재를 scaleb −12~+12 전수 확인 / 스케일 보존 / NaN·Inf 거부 / `-0`→`0`) · 왕복 항등 · `krw_floor` ROUND_DOWN 고정 + `abs(결과) ≤ abs(입력)` · `qty_floor` 격자 1·1e-8 + **lot_step 스케일 보존** + 결과 ≤ 입력 · `usd_budget` = `V/(rate*1.005)` 수치 일치 + 버퍼가 예산을 줄임 · `to_kst_text`/`from_kst_text`/`utc_to_text` naive 거부 + DST 타임존 안정성 · **70 tests / 318 passed**. property 테스트는 `strategies.py`와 함께 S32-3에서 추가 | 02 §5 [DD-02-9·10·15], 03 §3.1 | ☑ | `feat(core): Decimal·화폐·시각 직렬화 규약 구현` |
| **S02-3** | 식별자 체계 `core.ids` | `src/omra/core/ids.py`, `tests/unit/core/test_ids.py` | `new_id()` 10⁶회 유일 + 단조(발급 순서 == 사전식 정렬) · `Market` enum 5값 · `instrument_key`/`parse_instrument_key` 왕복 항등(`KRW-BTC` 하이픈 보존, 첫 콜론 분리) · 실패 5형이 전부 `IdentifierError` · **349 passed** | 02 §3.1~§3.2 [DD-02-1·2] | ☑ | `feat(core): 단조 ULID와 종목 식별자 체계 구현` |
| **S02-4** | 시각 추상 `core.clock` | `src/omra/core/clock.py`, `tests/unit/core/test_clock.py` | `Clock` ABC(`now_utc`·`now_kst`·`sleep_until`·`sleep_for`) · `SystemClock` 오프셋 +09:00 고정 · `SimClock` 후퇴 거부·naive 거부·벽시계 경과 ≈ 0 · `sleep_until` 과거 시각 즉시 반환 · **368 passed** | 02 §8 [DD-02-11·21] | ☑ | `feat(core): 결정론적 Clock 시간축 구현` |
| **S02-5** | 틱사이즈 규칙 `core.tick` | `src/omra/core/tick.py`, `tests/unit/core/test_tick.py`, `tests/property/test_inv_tick.py` | `TickRuleId` 4값 · `krx_etf_5`(5원)·`usd_penny`($0.01, <$1 거부) 확정 구현 · `krx7`·`upbit` 사다리는 **[확인 필요] 빈 코드 상수 + strict xfail 경계 테스트 2종**으로 봉인 · `snap_buy(p) ≤ p ≤ snap_sell(p)` / `is_aligned(snap_*(p))` / `next_up(p) > p` / `next_down(next_up(p)) == snap_buy(p)` property · synthetic 비정렬 구간 경계 과점프·역방향 격자·구간 수 기반 `ticks_between` 검증 · 비격자 끝점·비-Decimal·nonfinite·0 이하·큰 유한 Decimal 정밀도 방어 · **103 collected(101 passed, 2 xfailed) / 전체 469 passed, 2 xfailed** | 02 §6 [DD-02-7·8] | ☑ | `feat(core): 호가단위 정규화와 틱 산술 구현` |
...
| **S03-4** | `Order`·`Fill` 모델 | `src/omra/core/models.py`(3차), `tests/unit/core/test_order_fill.py` | `Order` validator 4종(qty>0 + lot_step 격자 / limit_price 필수·금지 조합 / `is_aligned` / aware datetime) · 갱신 실패 원자성·Pydantic fields-set 보존 · `transition_to()` 외 status 대입·Pydantic trusted API·저수준 우회 금지 아키텍처 테스트 · `filled_qty`를 필드로 두지 않음(fills 합산 파생) · `Fill` frozen·검증된 `model_copy`·qty>0·price>0 · **37 tests / 전체 688 passed, 2 xfailed** | 02 §7.3 [DD-02-5] | ☑ | `feat(core): Order와 Fill 도메인 모델 구현` |
