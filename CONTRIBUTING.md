...
- 자동화 branch 이름에는 `codex` 문자열을 사용하지 않는다.

## 1.1 자동 릴레이 카드 규칙

- GitHub Project #3의 이슈 하나를 한 대화 세션의 유일한 구현 단위로 삼는다.
- 구현 전에 이슈를 `In Progress`로 옮기고, 검증된 PR 병합과 이슈 종료가 끝난 뒤에만 `Done`으로 옮긴다.
- 카드 완료 댓글에는 branch·commit·PR, 실제 검증 결과, 운영상 미완료 gate, 다음 세션의 단일 후보와 정본 근거를 남긴다.
- 같은 대화 세션에서 다음 카드를 착수하지 않는다. 다음 세션은 완료 댓글과 정본 문서에서 상태를 복원하며 이미 끝난 탐색을 반복하지 않는다.
- 자동 진행 권한은 안전·자격증명·외부 production·파괴적 작업의 승인 경계를 넓히지 않는다. 그런 경계가 없으면 카드별 추가 승인을 요구하지 않는다.
- 세션을 새로 생성하는 기능이 실행 환경에 없으면 현재 세션은 릴레이 기록 후 종료한다. 이를 이유로 한 세션에 두 카드를 섞지 않는다.
